...
class AllegroRH(Allegro):
    def __init__(self):
        super().__init__()
        # Keep path convention aligned with other dexhands (shadow/inspire):
        # resolved by DexHand.urdf_path from workspace root.
        self._urdf_path = "data/assets/allegro_hand/allegro_hand_right.urdf"
        self.side = "rh"
        ...
        # Isaac-Lab Allegro RH asset in this repo uses r_* names without ".0":
        # base_link -> r_base_link, link_3.0_tip -> r_link_3_tip, joint_0.0 -> r_joint_0.
        def _rh_name(name: str) -> str:
            if name == "base_link":
                return "r_base_link"
            return "r_" + name.replace(".0", "")

        self.body_names = [_rh_name(n) for n in self.body_names]
        self.dof_names = [_rh_name(n) for n in self.dof_names]
        self.hand2dex_mapping = {k: [_rh_name(v) for v in vals] for k, vals in self.hand2dex_mapping.items()}
...
class AllegroLH(Allegro):
    def __init__(self):
        super().__init__()
        self._urdf_path = "data/assets/allegro_hand/allegro_hand_left.urdf"
        self.side = "lh"
