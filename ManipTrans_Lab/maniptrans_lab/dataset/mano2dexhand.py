...
# NOTE(ManipTrans_Lab): The original ManipTrans Mano2Dexhand retargeter runs an
# Isaac Gym sim to solve IK for hand pose → dexhand DoFs. ManipTrans_Lab targets
# Isaac Lab for training, but retargeting is a *preprocessing* step — keep using
# the original Isaac Gym script (from the source ManipTrans repo) to generate
# retargeted trajectories, then point this package's dataset loaders at the
# resulting .pkl files. We import gymapi lazily so the rest of the dataset
# package is usable under Isaac Lab without gym installed.
try:
    ...
    gymapi = gymtorch = gymutil = None
    _HAS_ISAAC_GYM = False
