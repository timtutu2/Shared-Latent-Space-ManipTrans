...
class ManipDataFactory:
    ...
    def auto_register_data(cls, directory: str, base_package: str):
        ...
        Tolerant of ImportError: heavyweight loaders (smplx / pytorch3d /
        chamfer_distance) may be missing on a user's machine, but the
        lightweight loaders (grab_demo_dataset) must still register.
        """
        import warnings
        for filename in os.listdir(directory):
            if filename.endswith(".py") and filename != "__init__.py":
                ...
                try:
                    ...
                    warnings.warn(
                        f"[ManipDataFactory] skipped {module_name}: {e}. "
                        f"Install the optional deps if you need this dataset."
                    )
