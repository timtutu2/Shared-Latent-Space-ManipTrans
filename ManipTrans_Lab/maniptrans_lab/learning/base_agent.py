...
class BaseAgent(torch.nn.Module):
    ...
    def train_model(self, max_samples, out_dir, save_int_models, logger_type):
        ...
        while self._sample_count < max_samples:
            ...
            print_iter = (self._iter % 50 == 0) or (self._sample_count >= max_samples)
            ...
            if (print_iter):
                ...
                if mp_util.is_root_proc():
                    progress = min(self._sample_count / max_samples, 1.0) if max_samples > 0 else 0.0
                    bar_len = 30
                    filled = int(bar_len * progress)
                    bar = "#" * filled + "-" * (bar_len - filled)
                    print("[{}] {:6.2f}% | iter {} | {:,}/{:,} samples".format(
                        bar, progress * 100, self._iter, self._sample_count, max_samples), flush=True)
