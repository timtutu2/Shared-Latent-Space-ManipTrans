...
class ObjCfg:
    ...
    def is_valid_clone(self, other):
        ...
        is_same &= self.disable_gravity == other.disable_gravity
        return is_same
...
class IsaacLabEngine(engine.Engine):
    ...
    def create_obj(self, env_id, obj_type, asset_file, name, is_visual=False, enable_self_collisions=True,
                   ...
                   color=None, disable_motors=False, disable_gravity=False,
        ...
        # Attach per-joint PD gains for articulated objects (used by
        # _build_articulated_prim when creating the ImplicitActuatorCfg).
        obj_cfg.stiffness = stiffness
        obj_cfg.damping = damping
    ...
    def get_gravity(self):
        """Read gravity from IsaacLab's SimulationCfg (default is [0, 0, -9.81])."""
        g = getattr(self._sim.cfg, "gravity", None)
        if g is None:
            return (0.0, 0.0, -9.81)
    ...
    def get_contact_forces(self, obj_id):
        ...
        if sensor is None:
            ...
            num_bodies = obj.data.body_link_pose_w.shape[1]
            forces = torch.zeros(
    ...
    def set_body_forces(self, env_id, obj_id, body_id, forces):
        if (env_id is None or len(env_id) > 0):
            ...
            obj = self._objs[obj_id]
            # IsaacLab's permanent wrench composer expects [n_envs, n_bodies, 3].
            # Keep a singleton body axis for both articulated and rigid objects.
    ...
    def get_palm_body_sim_idx(self, obj_id):
        """Sim-space (PhysX) link index of the palm / base_link for this obj.
        Equals 0 when the articulation root IS the palm; otherwise non-zero
        (happens when Isaac Sim's URDF Importer picked a non-wrist link as root,
        as in the bundled inspire_hand_right_lab.usd). Callers can use this to
        apply a palm-offset correction when setting the root pose so that the
        palm (not the arbitrary articulation root) lands at the desired world
        pose."""
        if not hasattr(self, "_palm_body_idx"):
            return 0
        return self._palm_body_idx[obj_id]
    ...
    def _parse_usd_path(self, asset_file):
        ...
        asset_ext = asset_ext.lower()

        # Default convention: replace input asset extension with .usd.
        if (asset_ext != ".usd" and asset_ext != ".usda"):
            usd_file = asset_root + ".usd"
        else:
            usd_file = asset_file
        ...
        # Fallback for assets exported under a subfolder named after basename,
        # e.g. ".../allegro_hand_right/allegro_hand_right.usd".
        asset_dir = os.path.dirname(asset_root)
        asset_base = os.path.basename(asset_root)
        nested_usd_file = os.path.join(asset_dir, asset_base, asset_base + ".usd")
        if os.path.exists(nested_usd_file):
            return nested_usd_file

        # Return the default path so downstream code raises a clear error path.
        return usd_file
    
    def _build_actuator_cfg(self, control_mode, stiffness=None, damping=None):
        """Build actuator config. `stiffness` and `damping` can be either None,
        a scalar, or a dict mapping joint-name regex to value (to match the
        reference ManipTrans ArticulationCfg format from env yaml)."""
        from isaaclab.actuators import ImplicitActuatorCfg, IdealPDActuatorCfg

        # If a custom stiffness/damping dict is supplied (e.g. from env yaml
        # articulation_cfg), build a single actuator group with those values.
        # The regex-keyed dict format is preserved; Isaac Lab matches each regex
        # against the joint_names_expr union to assign per-joint gains.
        if stiffness is not None or damping is not None:
            joint_names_expr = [".*"]
            if isinstance(stiffness, dict):
                joint_names_expr = list(stiffness.keys()) or [".*"]
            if (control_mode == engine.ControlMode.none):
                actuator_cfg = IdealPDActuatorCfg(joint_names_expr=joint_names_expr,
                                                  stiffness=stiffness or 0,
                                                  damping=damping or 0,
                                                  effort_limit=0)
            elif (control_mode in (engine.ControlMode.pos, engine.ControlMode.vel)):
                actuator_cfg = ImplicitActuatorCfg(joint_names_expr=joint_names_expr,
                                                  stiffness=stiffness,
                                                  damping=damping,
                                                  effort_limit=None)
            elif (control_mode in (engine.ControlMode.torque, engine.ControlMode.pd_explicit)):
                actuator_cfg = IdealPDActuatorCfg(joint_names_expr=joint_names_expr,
                                                  stiffness=stiffness,
                                                  damping=damping,
                                                  effort_limit=None)
    ...
    def _build_articulated_prim(self, env_id, obj_id, obj_cfg):
        ...
        rigid_props = sim_utils.RigidBodyPropertiesCfg(max_depenetration_velocity=10.0,
                                                       ...
                                                       max_linear_velocity=1000.0,
                                                       max_angular_velocity=1000.0,
                                                       disable_gravity=obj_cfg.disable_gravity)
        usd_asset_file = self._parse_usd_path(obj_cfg.asset_file)
        usd_cfg = sim_utils.UsdFileCfg(usd_path=usd_asset_file, 
                                       visual_material=visual_material,
        ...
        # Optional per-joint stiffness/damping from obj_cfg (set by the env
        # layer, typically from env yaml's articulation_cfg block).
        stiffness = getattr(obj_cfg, "stiffness", None)
        damping   = getattr(obj_cfg, "damping", None)
    ...
    def _build_order_tensors(self):
        ...
        self._palm_body_idx = []   # PhysX link index (sim-space) of the actual palm / base_link
        #                           per obj; used to correct root pose at reset when Isaac Sim
        #                           picked a non-wrist link as the articulation root.
        ...
        for obj_id in range(objs_per_env):
            ...
            if (obj_type == engine.ObjType.articulated):
                ...
                # ---- Detect root/palm mismatch in the USD articulation ----
                # Isaac Sim's URDF Importer sometimes picks a non-root link as the articulation
                # root (e.g. inspire_hand_right_lab.usd has R_thumb_proximal_base at sim[0] while
                # R_hand_base_link is at sim[1]). set_root_pos in Isaac Lab moves the PhysX root,
                # so we need to know the palm index to apply a post-reset offset correction.
                link_names = obj.root_physx_view.shared_metatype.link_names
                palm_idx = 0  # default: articulation root IS the palm
                for i, name in enumerate(link_names):
                    if ('palm' in name.lower()) or ('base_link' in name.lower()):
                        palm_idx = i
                        break
                if palm_idx != 0:
                    from util.logger import Logger as _Logger
                    _Logger.print(f"[RootPalm] obj_id={obj_id}: sim root='{link_names[0]}' != "
                                  f"palm='{link_names[palm_idx]}' (sim_idx={palm_idx}); "
                                  f"reset code should apply palm-offset correction.")

                # ---- Inspire hand DoF name-based mapping ----
                # Empirical evidence (tools/probe_joint_mapping.py) confirmed
                # that the shipped inspire_hand_right_lab.usd has joint names
                # that DO match their physical function — no scrambling. We
                # just need to map sim joint name → its position in the URDF
                # dof_names order (that's the order the caller's opt_dof_pos
                # is stored in). An earlier remap table circulated looked
                # different but was debunked by the probe; the identity-by-name
                # mapping below is correct.
                is_inspire = any("thumb_proximal_base" in n.lower() for n in link_names)
                if is_inspire:
                    metatype = obj.root_physx_view.shared_metatype
                    dof_names_sim = list(getattr(metatype, "dof_names", []) or [])
                    if dof_names_sim:
                        # URDF dof_names order (matches InspireRH.dof_names with R_ prefix).
                        # This is the order our caller's opt_dof_pos is in.
                        URDF_DOF_ORDER = [
                        ...
                        new_common2sim = []
                        for si in range(len(dof_names_sim)):
                            sim_name = dof_names_sim[si].split("/")[-1]
                            try:
                                urdf_idx = URDF_DOF_ORDER.index(sim_name)
                            except ValueError:
                                urdf_idx = dof_order_common2sim[si].item()
                            new_common2sim.append(urdf_idx)

                        new_sim2common = [0] * len(new_common2sim)
                        for si, ui in enumerate(new_common2sim):
                            new_sim2common[ui] = si

                        dof_order_common2sim = torch.tensor(new_common2sim, device=self._device, dtype=torch.long)
                        dof_order_sim2common = torch.tensor(new_sim2common, device=self._device, dtype=torch.long)
                        from util.logger import Logger as _Logger
                        _Logger.print(f"[InspireRemap] obj_id={obj_id}: identity-by-name "
                                      f"URDF→sim DoF mapping ({len(new_common2sim)} DoFs). "
                                      f"Caller passes values in InspireRH.dof_names order.")
            else:
            ...
            self._palm_body_idx.append(palm_idx)
